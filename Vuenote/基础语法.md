...
- 打包 
##### 安装
- webpack依赖node环境 (node为了正常执行代码，必须包含各种依赖包。npm用于管理包)
- vue cli依赖webpack3.6.0 全局安装[npm install webpack@3.6.0 -g] 局部安装[npm install webpack@3.6.0 --save-dev],终端中输入webpack获得的都是全局webpack
##### 使用
- 使用webpack打包[webpack ./src/main.js(入) ./dist/bundle.js(出)] html中直接引用bundle.js
- 配置好直接打包[ webpack ]
webpack.config.js 中设置入口和出口(动态获取路径)
一旦准备使用nord就先进行初始化[npm init]生成package.json,若存在要使用的依赖[npm run install]
- [npm run build]
...
##### 创建项目
- 初始化 cli2 [vue init webpack 项目名] 有很多选项
- 初始化 cli3 [vue create 项目名]
- ES(js)-Lint 代码规范
- e2e(e to e)端到端测试
##### 初始化项目
- [npm run dev]
